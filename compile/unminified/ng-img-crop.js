 ...
 * License: MIT
 *
 * Generated at Friday, May 15th, 2015, 4:01:48 PM
 */
(function() {
'use strict';
...
crop.factory('cropAreaFixed', ['cropArea', function(CropArea) {
  var CropAreaFixed = function() {
    CropArea.apply(this, arguments);
  ...
  CropAreaFixed.prototype = new CropArea();

  CropAreaFixed.prototype._calcRectangleCorners = function() {
  ...
  CropAreaFixed.prototype._calcRectangleDimensions = function() {
    var size = this.getSize();
    var se = this.getSouthEastBound();
    return {
      left: size.x,
  ...
  CropAreaFixed.prototype._isCoordWithinArea = function(coord) {
  ...
  CropAreaFixed.prototype._isCoordWithinResizeCtrl = function(coord) {
  ...
  CropAreaFixed.prototype._drawArea = function(ctx, center, size) {
    ctx.rect(size.x, size.y, size.w, size.h);
  };

  CropAreaFixed.prototype.draw = function() {
    CropArea.prototype.draw.apply(this, arguments);
  ...
  CropAreaFixed.prototype.processMouseMove = function(mouseCurX, mouseCurY) {
    var cursor = 'default';
    var res = false;
  ...
  CropAreaFixed.prototype.processMouseDown = function(mouseDownX, mouseDownY) {
  ...
  CropAreaFixed.prototype.processMouseUp = function( /*mouseUpX, mouseUpY*/ ) {
    if (this._areaIsDragging) {
      this._areaIsDragging = false;
      this._events.trigger('area-move-end');
    }
...
crop.factory('cropArea', ['cropCanvas', function(CropCanvas) {
  ...
  CropArea.prototype.setSizeByCorners = function(northWestCorner, southEastCorner, type) {
    var maxWidth = this._ctx.canvas.width,
      currentWidth = southEastCorner.x - northWestCorner.x,
      w = southEastCorner.x - northWestCorner.x,
      h = southEastCorner.y - northWestCorner.y;

    if (type == 'fixed') {
      w = currentWidth < maxWidth ? currentWidth : maxWidth;
      h = w * this._minSize.h / this._minSize.w;
  ...
  CropArea.prototype.setMinSize = function(size, type) {
    this._minSize = this._processSize(size, type);
    this.setSize(this._minSize);
    this._dontDragOutside();
  };
  CropArea.prototype.setMinSizeArea = function(size) {
    this._minSize = size;
    this.setSize(this._minSize);
    this._dontDragOutside();
  };
  ...
  CropArea.prototype._dontDragOutside = function() {
    ...
    if (se.x > canvasW) {
      newSize.x = canvasW - s.w;
    }
    if (se.y > canvasH) {
      newSize.y = canvasH - s.h;
    }
  ...
  CropArea.prototype._processSize = function(size, type) {
    ...
    var calSize = {
      x: Math.max(this._minSize.x, size.x) || this._minSize.x,
...
crop.factory('cropHost', ['$document', 'cropAreaCircle', 'cropAreaSquare', 'cropAreaRectangle', 'cropAreaFixed', 'cropEXIF', function($document, CropAreaCircle, CropAreaSquare, CropAreaRectangle, CropAreaFixed, cropEXIF) {
  /* STATIC FUNCTIONS */
  ...
  return function(elCanvas, opts, events) {
    ...
    // Resets CropHost
    var resetCropHost = function(resultImageSize) {
      if (image !== null) {
        ...
        var w, h, x, y;
        if (theArea.type == 'fixed') {
          w = parseInt(resultImageSize.w / (image.width / canvasDims[0]), 10);
          h = parseInt(resultImageSize.h / (image.height / canvasDims[1]), 10);
          x = 0;
          y: 0;
        } else {
          w = Math.min(200, ctx.canvas.width / 2);
    ...
    var onDragOver = function(e) {
      ...
      e.preventDefault();
      angular.element(document.querySelector('.cropArea')).addClass('dragenter');
    }

    var onDragLeave = function(e){
      angular.element(document.querySelector('.cropArea')).removeClass('dragenter');
    }
    ...
    this.setNewImageSource = function(imageSource, resultImageSize) {
      image = null;
      resetCropHost(resultImageSize);
    ...
    this.setAreaMinSize = function(size) {
      ...
      if (!isNaN(size.w) && !isNaN(size.h)) {
        theArea.setMinSize(size,theArea.type);
        drawScene();
      }
    };
    ...
    this.getAreaType = function(type) {
      theArea.type = type;
    }
    this.setAreaType = function(type, resultImageSize) {
      var center = theArea.getCenterPoint();
      ...
      if (theArea.type == 'fixed') {
        curMinSize = {
          ...
          y: 0,
          w: 80,
          h: parseInt(resultImageSize.h / (resultImageSize.w / 80), 10)
        };
      ...
      if (type === 'square') {
        ...
        AreaClass = CropAreaFixed;
      };
      theArea = new AreaClass(ctx, events);
      theArea.setMinSize(curMinSize,type);
